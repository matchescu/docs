...
dataset~\cite{expdata2023} which we createdl from existing data 
\textcolor{green}{I Would specify here the original dataset used for building the control dataset}
. 
...
Amazon-GoogleProducts~\cite{vldb2010} benchmarked data sets. 
...
\textcolor{green}{eu zic ca prinde bine sa reiei si tabelul (partea stanga) - pe langa referinta - : baza de date (cele 3 si tiny-DS), ce atribute sunt, care e tipul lor (string/numeric/date) si cate entitati; mai mult, datasetul DBLP-Scholar nu a fost prins in experimentele tale - de ce?}
...
Let us take the following record from the `Buy' subset as an example (see Table \ref{tab:buy-record}).
...
The following entry corresponds to the record in Table \ref{tab:dg1-record}.
...
\begin{table}[ht]
    ...
    \begin{tabular}{|l|l|l|l|}
        ...
        \hline
    \end{tabular}
    \caption{An example record of the \textcolor{green}{???} subset}
    \label{tab:dg1-record}
\end{table}

The corresponding DG2 record would then look as follows (see Table \ref{tab:dg2-record}).
...
\textcolor{green}{to disscuss their conclusion!}
...
This situation leads to presenting our \textbf{first invariant condition candidate.
...
While in the context of a small control dataset it may not seem likely that this
would raise concerns, consider that for practical applications the size of the
ground truth is often several orders of magnitude smaller than the size of the
input.
...
resolution model used to interpret result also matters in the case of
probabilistic recall.
Other algorithms may provide the conditions for this phenomenon using their own
input configurations.

In the end, pairwise metrics seem to be a more accurate way of interpreting
results that is similar to the probabilistic metrics.
...
candidate for our second invariant condition.}
...
The conjecture here is that \textbf{the best generic configuration balanced between
precision and recall of an entity resolution algorithm will be found within the
agreement area between ARI on one side and TWI or cluster metrics on the other.}
...
precision.
Finally, the highest harmonic mean between precision and recall is well within
the target interval we envisioned at the end of the
Subsection~\ref{subsec:experiment-mini-buy}.
Furthermore, the ARI, TWI and the cluster metrics correlate.
...
probabilistic precision and recall is well within the target interval.
...
The configuration $t=0.32$ which has the highest F1 score is still within the
predicted interval.

\textcolor{green}{add some words about the limitations (e.g. complexity)}