...
\]
    ...
    It is comprised of a subset of the Abt-Buy dataset\cite{vldb2010}.
    Namely, we have chosen ten items from the Buy dataset that help us highlight
    the differences in interpretation that arise when using different models.

    In order to introduce similar entity references that will be processed by
    the entity resolution task, we split the Buy table into two tables having
    the following columns from the original table:
    
    \begin{enumerate}[label=\textbullet~Table~\arabic*,leftmargin=*]
        \item will contain the `name',`manufacturer' and `price' columns
        \item the second table will contain the data in the `description',`name' columns from the original table.abc
    \end{enumerate}
