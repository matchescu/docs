...
\]
    ...
    To understand why all of the above is not just jibber-jabber, we propose an
    experiment that shows how entity resolution results vary according to the
    mathematical model chosen for an entity resolution function.

    In our methodology for elaborating the experiment we set aside any notions
    about input sample size so that blocking and filtering don't play a part in
    our results.

    Furthermore, since we are not concerned with the actual score, but with
    showcasing the differences induced by using different mathematical models
    for our entity resolution tasks, we will use a synthetic data set derived
    from the DBLP dataset\cite{vldb2010}.
    The data is derived by splitting the dataset into two datasets by column and
    keeping two common columns (the `id' and `title').
    The `authors' column will be assigned to the first dataset and the remaining
    two columns will be assigned to the second dataset.

    For simplicity, our extraction traits will produce one value per column per
    record.
    This means that each record in each dataset is going to be an entity
    reference.
    The entity references in the first dataset are going to be comprised of
    three attributes, while the entity references from the second dataset are
    going have four attributes each.

    For example, let's say we were given this data from the DBLP dataset.

    \begin{tabular}{lllll}
        id& title & authors & venue  & year \\
        \hline
        journals/sigmod/Liu02 & Editor's Notes & Ling Liu & SIGMOD Record & 2002 \\
    \end{tabular}

    We then generate dataset 1 to be the following:

    \begin{tabular}{lll}
        id& title & authors \\
        \hline
        journals/sigmod/Liu02 & Editor's Notes & Ling Liu  \\
    \end{tabular}

    Finally we generate dataset 2 to be the following:

    \begin{tabular}{lllll}
        id& title & venue & year \\
        \hline
        journals/sigmod/Liu02 & Editor's Notes & SIGMOD Record & 2002 \\
    \end{tabular}


    The first dataset contains the entity reference (
    \texttt{`journals/sigmod/Liu02',`Editor's Notes',`Ling Liu'}).
    The second dataset contains the entity reference (
        \texttt{`journals/sigmod/Liu02',`Editor's Notes',`SIGMOD Record',`2002'}
    ).
