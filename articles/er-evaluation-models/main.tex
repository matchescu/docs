...
\]
    ...
    However, we are rotating the data on the column axis so that the `id' column
    is always last.
    ...
    When the Jaccard similarity threshold is set high ($t \geq 0.7$), we end up
    with few matches, but those matches are true.
    As we lower the threshold ($0.7 \leq t < 0.85$), recall increases and
    precision does not drop.
    However, when we lower the threshold too much ($t < 0.18$) we start losing
    precision.
    ...
    However, the precision metric shows us the maximum score.
    The metric itself isn't wrong, it just does not capture the correct matches
    that are in surplus.

    The more upsetting fact is that the duplicate true positives are the cause
    of a rise in precision when we lower the Jaccard threshold.
    That leads to a tainted F1 score which is the metric that most people judge
    entity resolution by.

    This shows us that while statistical metrics are comfortable to use and easy
    to understand, they might not be completely transparent or perfectly show
    the performance of the entity resolution algorithm.
    In fact, the sharp drop in recall as the Jaccard threshold rises and the
    precision maintains its value might be the only clue that points to the
    existence of ``duplicate'' true positives.
    We can make these inferences only because we have intimate knowledge of the
    algorithm.
    If we didn't we wouldn't be able to confidently assess its performance.
    ...
    In the `Abt-Buy' setup, the items in the ideal mapping will are paired up
    into clusters in the ground truth.
    The rest of the items in the input data are added as singleton clusters.

    The results for this model are better for two reasons.
    Firstly, the diversity of metrics allows us to see different facets of the
    same thing.
    Clustering metrics and pair metrics show data that is more similar to what
    we're used to when using the statistical model.
    The three models give us better insight into what we might expect to be a
    part of the match.
    So the first point is that the model is more informative.
    ...
    happening.
    Take for example the problem of extra true positives.
    We can very quickly identify that problem by looking at any one of the three
    provided indexes in coroboration with the pairwise metrics.
    The only possible interpretation of a high pairwise similarity that happens
    simultaneously with low cluster similarity is extra matching pairs.

    Another observation is how the statistical metrics compare with the pairwise
    metrics in that the shapes of the graphs are very similar.
    The additional information in the algebraic model is that, in fact, when we
    look at thresholds nearing zero pairwise recall also decreases.
    The reason behind that is the overwhelming amount of duplicates that is
    being returned by the algorithm.
    That is, the ground truth becomes much smaller than the entity resolution
    result and, therefore, the pairwise recall decreases because there are many
    paris in the result that are not matched in the ground truth.
    Were we to still use statistical recall, we'd be under the impression that
    we're getting a perfect score as we're lowering the Jaccard threshold
    towards zero.
    
    Finally, the algebraic model exposes clustering metrics.
    These metrics suggest that the entity resolution task did not perform
    perfectly.
    Moreover, they show a drop in quality as we increase the Jaccard threshold.
    This is in line with our intuition, however it is not in line with the
    pairwise metrics or the statistical metrics.

    The fundamental takeaway from this experiment is that the algebraic model
    provides a much more transparent and accurate picture over the performance
    of an entity resolution algorithm.
    ...
    To replicate our findings on our very small dataset, we propose running the
    same algorithm, in the same way on the `Abt-Buy' dataset.
    The purpose here is to see which of the two models has steered us closest to
    the optimal settings for our algorithm.
    ...
    \end{figure}\label{algebraic-full}
