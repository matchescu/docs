...
\]
    ...
    increasing, signaling that the contents of the entity resolution task's
    results are not well aligned with the ground truth for lower values of $t$.
    Then, if we look at the cluster precision, we see that increasing the
    similarity threshold actually decreases cluster precision.
    once we take longer prefixes into account.
    ...
    We can also see that the pairwise metrics (in particular pairwise recall)
    ...
    The small dataset shows us the potential advantages of using the algebraic
    model over using the statistical model.
    Now it's time to see whether those observations make a difference on a 
    standard entity resolution benchmarking dataset.
    ...
    extremes, the Talburt-Wang index is monotonically increasing and all of the
