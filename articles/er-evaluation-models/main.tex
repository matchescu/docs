...
\begin{document}
    ...
    We also leave aside any classification based on number of sources.
    ...
    For instance, we may what are the perspectives available to us when we want
    ...
    This lead to using better measures that describe the outcomes more wholly
    and more objectively.
    This indicates both that there is more than one lens to view entity
    ...
    \begin{defn}
        An \textit{data source} represents a sequence of decoded messages
        that originate somewhere and can be processed by a computer program that
        reads them over a communication channel.
    \end{defn}
    ...
    The notion that's the most similar to that of a trait is the pattern
    ...
    To avoid confusing traits and attributes, it's easy to think about traits
    ...
    to matching and clustering.

    However, because extraction plays a big part in how data is structured
    within any entity resolution process, we start formalizing entity resolution
    by formalizing the data extraction using the notions from
    ...
    Record linking tasks that use two or more data sources can embed the an
    ...
    This finally allows us to formalize entity resolution itself as a function:
    ...
    these sequences must be the ideal one.
    ...
    This can be applied to meet our needs by leveraging the input domain $Ref$
    ...
    It can't be incomplete or faulty, by definition.
    ...
    given the input sequence of pairs it can't produce two partitions over $Ref$
    on two separate occasions).

    If we have a ground truth expressed as an iterable sequence of matching
    pairs and we know the input $Ref$ domain that's used for entity resolution,
