...
\begin{document}
    ...
    Entity resolution has been classified according to the number of sources and
    the technological choice underlying the solution.
    Whether entity resolution solutions are implemented using machine learning
    or are rule based is not of particular interest in this paper.
    The more interesting classification in our opinion concerns whether entity
    resolution algorithms are matching algorithms or clustering ones.
    The family which an entity resolution solution belongs to is interesting on
    multiple levels.
    
    Firstly, there are theoretical models that deal with entity matching and
    there are clustering models.
    Matching is more related to information retrieval whereas clustering is a
    problem domain with many ramifications including, but not limited to,
    knowledge management, complex networks or computer vision.
    So finding a terminology that applies to both potentially opens up a bridge
    between entity resolution and findings in those fields.
    This is especially true because of entity resolution's fundamental nature:
    there are very few things more fundamental to humans than identifying real
    world objects.

    Secondly, theoretical models are merely ways of describing something with
    some theoretical tools (usually mathematical in nature):an idea, an
    occurence or an entire phenomenon.
    They influence our perception over that which they describe because of how
    they describe it.
    Entity resolution models are no different.
    They can describe entity resolution as a matching problem confining us to
    think about what is happening in the entity resolution process as a matching
    operation.
    Or they can describe entity resolution as a clustering problem only allowing
    interpretations that focus on the composition and size of clusters.
    
    In this paper we want to discuss broadly the suitability of an entity
    resolution solution through the lens of two different models.
    We chose one model from the matching family and another from the clustering
    family of entity resolution models for this discussion.
    In order for the discussion to take place, we require some common terms that
    describe things which are specific to entity resolution and how they apply\
    to the broader notions of matching and clustering.
    That entails introducing a framework where we define these notions.
    This framework also helps highlighting the differences between matching and
    clustering models of entity resolution.

    We are also interested in linking entity resolution quality metrics to
    model families.
    ...
    We are interested in the interplay between two model, one from each family.
    ...
    The Fellegi-Sunter model is the archetypal model for entity matching.
    ...
    resolution quality evaluation is highly influenced by the probabilistic
    approach used with this model.
    ...
    This paper attempts to find out if there are any conditions that pertain to
    the entity resolution model itself.
    ...
    to data clustering afficionados.
    Other clustering models, including a particular version of the Stanford
    Entity Resolution Framework model, use the same metrics.
