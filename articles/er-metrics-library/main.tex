...
\begin{document}
    ...
    The library we are introducing features implementations of various
    ...
    The paper speaks about accuracy and contamination similarly to how we
    describe true and false positives today.
    ...
    requiring users to use labels or adapt their own data to the strictures of
    the API, our library takes full advantage of Python protocols by leveraging
    protocols, such as the \texttt{Iterable} and \texttt{Hashable}.
    ...
    Since sets operate on \texttt{Hashable} items, virtually any instance that
    implements the \texttt{Hashable} protocol can be passed as input to the
    library.
    
    Adhering to the \texttt{Hashable} protocol has a few less obvious advantages
    over using labels to compute metrics:
    \begin{itemize}
        \item it works out of the box with basic types such as numbers or
        strings, but also with tuples or lists;
    ...
    The technology choice deserves special attention because of a limitation it
    imposes on the design, too.
    ...
    \texttt{Hashable}.
    This entails an obligation to verify that, when partitions over a set are
    expected as the input arguments of a library function, that function will
    verify that the input arguments are partitions over the same set.
    ...
    library without substantial gain.
