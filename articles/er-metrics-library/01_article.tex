...
\begin{document}
    ...
    \author{
        ...
        \affiliation{\sup{1}Faculty of Mathematics and Computer Science, Babes-Bolyai University}
    ...
    ER has the role of automatically building the student profile
    ...
    Maintaining highly qualitative ER is necessary in order to
    avoid the presence of misleading information in decision processes.
    ER system adoption and maintenance cannot be performed
    without measuring ER quality.
    ...
    ER systems partly function as information retrieval systems,
    ...
    metrics adapted for ER.
    It's also important to acknowledge the distinct mathematical models
    underlying ER compared to information retrieval.
    ...
    % \begin{enumerate}
        % \item 
        embracing an OpenSource licensing model,
        % \item 
        efficient implementation using state of the art libraries under
        a very popular platform, and
        % \item 
        a design that is agnostic to the ER implementation.
    % \end{enumerate} 

    % The outline of this article is as follows.
    After this introduction, we overview two existing mathematical models for
    ER which, in spite of their age, are still widely used and
    ...
    The work of the InfoLab at Stanford on their Stanford ER
    Framework\cite{Ben2009Swoosh} and that of the Center for ER
    ...
    ER in syntheses on the subject\cite{vldb2010,hitesh2012,Tal11}.
    Metrics that measure aspects of clustering seem to be used more frequently
    to measure ER quality as time passes.
    ...
    Lastly, there are numerous systems that perform ER available.
    ...
    for effectively evaluating ER tasks may be absent\cite{ereval}.
    ...
    their close relationship with the algebraic model for ER.
    All of these metrics expect input arguments that are partitions over a set.
    Moreover, the ground truth and the ER result must be
    ...
    % \[ \varDelta(A, B) = \frac{|A|\cdot|B|}{\varPhi{\ointerval{A}{B}}^2}\textrm{, where}\]
    ...
    equivalence classes of each partition generated by the ER
    ...
    PPJoin\cite{ppjoin} entity matching algorithm to perform ER.
    ...
    interval $\rinterval{0}{1}$ at increments of $0.01$ (ass we show in Figures \ref{fig:results1}-\ref{fig:results3}).
    All the plots of the outcome available
    \href{https://github.com/matchescu/py-resolve-metrics}{on GitHub}.
    ...
    ER task from interefering with collecting the CPU profiling
    data.
    ...
    ER algorithm performs clustering.
    ...
    rooted in the mathematical models fundamental to ER.
    By doing so the library ensures that data is represented in a way that is
    specific to ER and not specific to another scientific problem
    or to a particular ER system or task.
    ...
    The ER models we have touched upon so far support many more
    ...
    % \begin{itemize}
        % \item 
        additional cluster comparisons, such as the Closest Cluster $F_1$,
        the MUC $F_1$, $B^3 F_1$ and the $CEAF F_1$~\cite{hitesh2012}, and
        % \item 
        additional Rand-like indexes~\cite{warrens2022understanding}.
    % \end{itemize}
