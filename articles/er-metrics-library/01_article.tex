...
\begin{document}
    ...
    The plots of the outcome are available
    \href{https://www.cs.ubbcluj.ro/~andrei.olar/article-data/er-metrics-library/#sample-data-metrics}{online}(\cite{sampleplots2024}).
    ...
    metrics.
    The performance graphs and call tables are available
    \href{https://www.cs.ubbcluj.ro/~andrei.olar/article-data/er-metrics-library/#performance-data}{online}~(\cite{perfgraphs2024}).

    We note that according to the `Fellegi-Sunter Flamegraph', computing the
    Fellegi-Sunter metrics takes up less CPU than validating the input data.
    The `Fellegi-Sunter Metrics CPU Stats' table provides more details into how
    inexpensive the probabilistic metrics really are.

    The same cannot be said about the metrics that work with partitions over an
    input set.
    Computing the algebraic metrics clearly dominates the `Algebraic Model
    Flamegraph'.

    In the `Algebraic Metrics' table we can see a breakdown of the most
    computationally intensive tasks.
    The pairwise and cluster metrics are not computationally intensive.
    ...
    This is due to the function being executed nine times (once for computing each
    metric).
    We take note to improve the performance of the function.
    ...
    This paper introduced a library for evaluating entity resolution results that
    ...
    \begin{itemize}
        \item additional cluster comparisons, such as the Closest Cluster $F_1$,
